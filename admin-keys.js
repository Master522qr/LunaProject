(() => {
  ...
  function render() {
    ...
    if (!rows.length) {
      body.innerHTML = '<tr><td colspan="6" style="text-align:center;padding:24px;color:var(--text-muted)">Ключей пока нет</td></tr>';
      return;
    }
  ...
  async function loadKeys() {
    const { data, error } = await sb.from('license_keys')
      .select('id,code,duration_days,is_used,used_by,used_at,created_at')
